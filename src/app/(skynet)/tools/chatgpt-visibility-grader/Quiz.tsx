...
const STORAGE_KEY = "skynet:chatgpt-visibility:v1";
...
export default function Quiz() {
  ...
  const roadmap = useMemo(() => collectRoadmap(answers), [answers]);
  ...
  async function copyRoadmap() {
    try {
      const lines = [
        `ChatGPT Visibility Grader — Score ${totalScore}/100 (${bucket.label})`,
        "",
        "Roadmap:",
        ...roadmap.map((r, i) => `${i + 1}. ${r.move}`),
      ];
    ...
    return (
      ...
      >
        ...
        <p className="mt-2 text-sm text-[var(--ink-faint)]">
          Checking 9 answers across 4 categories.
        </p>
      </div>
    );
  }
  ...
  if (phase === "result") {
    if (!unlocked) {
      return (
        <div ref={cardRef}>
          <EmailGate
            toolSlug="chatgpt-visibility-grader"
            toolName="ChatGPT Visibility Grader"
            promise="your full score breakdown + visibility roadmap"
    ...
    return (
      <div ref={cardRef}>
        ...
        <div className="mt-4 flex justify-center">
          <ToolUsage slug="chatgpt-visibility-grader" />
        </div>
      </div>
    );
  }
...
type ResultProps = {
  ...
  roadmap: { id: string; category: Category; move: string }[];
  onRestart: () => void;
  onCopy: () => void;
    ...
    <div className="overflow-hidden rounded-3xl border border-[rgba(26,26,26,0.12)] bg-[var(--cream-2)] backdrop-blur-md">
      ...
      <div className="grid grid-cols-1 gap-8 px-7 py-8 md:grid-cols-[1fr_1.2fr] md:px-12 md:py-10">
        ...
        <div>
          ...
          <p className="mb-3 text-xs font-semibold uppercase tracking-[0.2em] text-[var(--ink-faint)]">
            Your roadmap ({roadmap.length} moves)
          </p>
          <ul className="space-y-3">
            {roadmap.slice(0, 8).map((r, idx) => (
              <li
                key={r.id}
            ...
            {roadmap.length === 0 && (
              <li className="text-sm text-[var(--ink-2)]">
                No gaps found in your answers — keep this current as your brand
                grows.
              </li>
            )}
          </ul>
      ...
      <div className="border-t border-[rgba(26,26,26,0.12)] bg-[var(--cream-2)] px-7 py-7 md:px-12 md:py-8">
        <div className="flex flex-col gap-3 sm:flex-row sm:flex-wrap">
          <a
            href={`${CAL_URL}?utm_source=chatgpt-visibility-grader`}
            target="_blank"

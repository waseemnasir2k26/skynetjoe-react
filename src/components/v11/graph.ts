/* ============================================================
   ...
   (v11). One static graph (nodes = buildings, edges = light-bridges)
   drives BOTH the three.js wireframe geometry (SceneObjects.ts) AND
   the inline SVG facade fragments (FacadeFragment.tsx) — the
   "twin-source silhouettes" mechanic. No build script: both
   consumers import this module and derive their output at module
   evaluation / render time, so 2D and 3D can never drift.

   Ported verbatim from waseemnasir-react's aicity-signalgrid/graph.ts
   (positions/heights are structural code values, not user copy — the
   truth gate applies to rendered strings, which live in tokens.ts).
   CLIENT_NODES ids are renamed to match the reworded WORK entries in
   tokens.ts (same real cases, digit-free copy).
...
export type GraphNode = {
  ...
  powerOnAt: number;
  /** dashed silhouette (demo build — spec-demo truth made visual) */
  dashed?: boolean;
...
export type GraphEdge = {
  ...
  honest?: boolean;
  /** The Leak — pulses die mid-bridge, dimmed rust, no amber */
  broken?: boolean;
};
...
/* ---- Site B service blocks (TOWER FLYBYS band 32-50%) ---- */
...
/* ---- WORKS BOULEVARD (client towers, 64-82% band) — demo tower dashed ---- */
...
export const EDGES: GraphEdge[] = [
  ...
  // The Leak — dies mid-bridge, dimmed rust only, no red/amber.
  {
    id: "leak",
    from: "exchange",
    to: "the-leak",

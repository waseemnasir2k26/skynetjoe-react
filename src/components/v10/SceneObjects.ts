...
/* ============================================================
   SCENE OBJECTS — plain three.js builders (no react-three fiber libs/*, forbidden
   in this repo per this route's build brief). Ported from
   waseemnasir-react's components/aicity-altitude/SceneObjects.ts: same
   {group, update, dispose} builder-pattern contract, same instancing
   discipline, rebranded materials (terracotta night-window glow instead
   of jade).

   District windows and tower scale-Y are activated by ordinary
   InstancedMesh matrix writes, but ONLY for the instances belonging to
   the district that just crossed its threshold, and ONLY for a short
   500ms power-on animation after the crossing (then writes stop
   entirely) — steady-state per-frame cost is zero matrix writes.
   Bridges (below) use a real custom ShaderMaterial — that surface is
   small (<=24 instances, 2 uniforms).
   ============================================================ */
...
/* ============================================================
   CLOUD DECK — one plane, one custom ShaderMaterial (fbm-lite value
   noise, 4 octaves), horizontal above the corridor. Camera punches
   through it during the 8-16% band.
   ============================================================ */
...
/* ============================================================
   DESCENT DISTRICTS — the four service districts you fall past. ONE
   InstancedMesh for every tower across all four districts, ONE for
   every window. Each district "wakes" (tower scale-Y 0.85->1.0, its
   windows power on bottom-to-top) the moment scroll progress crosses
   that district's altitude-band threshold. Building heights below are
   explicitly decorative — never tied to any metric shown on a placard.
   ============================================================ */
...
/* ============================================================
   INTERCONNECTION — light-bridges. ONE InstancedMesh of thin beams strung
   between adjacent districts, ONE custom ShaderMaterial: a moving
   gaussian "packet" of full-brightness terracotta glow travels each beam,
   phase staggered per instance so a pulse visibly leaves the district you
   just read and arrives at the one you're falling toward.
   ============================================================ */
export function buildBridges(): SceneObject {
  ...
  // next, matching the canonical relay (intake -> n8n -> GHL -> dispatch
  // portal/content), ~4-6 beams, well under the <=24 instance budget.
  ...
  const mat = new THREE.ShaderMaterial({
    ...
    vertexShader: /* glsl */ `
      ...
      // six faces, so "position along the beam" is derived from
      // local-space position.z instead (box spans z in [-0.5, 0.5]
      // before the instance scale) — consistent on every face.
...
/* ============================================================
   STREET + TOUCHDOWN DOOR — final landing beat. A ground plane and a lit
   doorway the dock CTA sits behind thematically.
   ============================================================ */

...
/* ============================================================
   DESCENT CANVAS — plain three.js, mounted imperatively in a useEffect.
   No react-three fiber libs/* (forbidden in this repo per this route's build
   brief — v8's r3f/rapier stack is a different, unrelated route). Ported
   lifecycle verbatim from waseemnasir-react's
   components/aicity-altitude/AltitudeCanvas.tsx: fixed, full-viewport,
   sits behind the real HTML content (no CLS, never captures scroll —
   native scroll drives the camera via `progress`, read every rAF tick,
   no React re-renders), DPR capped at 1.5, dispose on unmount, and a
   webglcontextlost listener that bubbles up to the static fallback.
   ============================================================ */
export default function DescentCanvas({
  ...
  useEffect(() => {
    ...
    scene.fog = new THREE.Fog(C.skyDark, 20, 46);
    ...
    const ambient = new THREE.AmbientLight(0x2e2015, 0.34);
    const sky = new THREE.DirectionalLight(C.ink, 0.2);

...
/* ============================================================
   ...
   Ported verbatim (geometry only, no copy) from waseemnasir-react's
   components/aicity-altitude/CameraPath.ts. A single continuous DESCENT
   from above a cloud deck down onto a street-level dock, one waypoint per
   scroll band (7 bands = 7 waypoints, matches SCENES.length in
   V10Client.tsx so posCurve.getPoint(progress) always lands the camera in
   the band the HTML is currently showing).

   Note on scale: altitude in meters (2400m -> 0m) is a purely decorative
   HTML overlay (see AltimeterRail in V10Client.tsx) driven straight off
   scroll progress — it is NOT the camera's actual world-space Y. The
   camera moves through a small world-unit range (~1.5 to ~11 units) so
   geometry, fog and the fixed clip planes stay in proportion.
   ============================================================ */

...
const nextConfig: NextConfig = {
  ...
  output: "standalone",
  // No remote next/image hosts in use (Meta Pixel is a plain <img>); avif/webp
  // negotiation only. Add remotePatterns here if a CDN/remote <Image> lands.
  images: {
  ...
  async redirects() {
    return [
      // Host canonicalization — www → apex (301). Prevents duplicate-content
      // indexing on www.skynetjoe.com. robots.ts host hint alone is not enforcing.
      {
        source: "/:path*",
        has: [{ type: "host", value: "www.skynetjoe.com" }],
        destination: "https://skynetjoe.com/:path*",
        permanent: true,
      },
      ...wpRedirects,
